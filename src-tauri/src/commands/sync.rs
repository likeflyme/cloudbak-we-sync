...
fn upload_one_blocking(client: &reqwest::blocking::Client, base_url: &str, sys_session_id: i32, root: &Path, file_path: &Path) -> Result<()> {
    ...
    let form = reqwest::blocking::multipart::Form::new()
        .text("dest_path", dest_path)
        .text("sys_session_id", sys_session_id.to_string())
        .text("overwrite", "true")
        // new: pass client mtime (milliseconds)
...
fn load_session_config(session_id: i32) -> Option<SessionConfig> {
    let base = crate::internal::app_paths::app_data_dir().ok()?;
    let path = base.join("sessions").join(format!("{}.json", session_id));
    let bytes = std::fs::read(path).ok()?;
    serde_json::from_slice(&bytes).ok()
}

fn save_session_config_inner(session_id: i32, cfg: &SessionConfig) -> Result<()> {
...
// --- Auto Sync Watcher ---
fn spawn_watcher(session_id: i32, root: PathBuf, base_url: String, token: Option<String>, cancel: Arc<AtomicBool>) -> Result<()> {
    use notify::{RecommendedWatcher, RecursiveMode, Watcher};
    if !root.exists() { anyhow::bail!("wx_dir not found"); }

    std::thread::spawn(move || {
        // blocking client in this thread
        let client = {
        ...
        // debounce map to avoid rapid re-uploads; simple last-change timestamp
        use std::collections::BTreeMap;
        use std::time::{Duration, Instant};
        let mut last_upload: BTreeMap<String, Instant> = BTreeMap::new();

        // load filters
        let filter_str = load_session_config(session_id).and_then(|c| c.sync_filters).unwrap_or_default();

        // notify v6: use callback -> forward into channel as Result<Event, Error>
        ...
        while !cancel.load(Ordering::Relaxed) {
            ...
            let evt = match res { Ok(e) => e, Err(_) => continue };
            let kind = evt.kind;
...
pub async fn start_sync(
    ...
    std::thread::spawn(move || {
        ...
        let cfg = load_session_config(sys_session_id);
...
#[tauri::command]
pub async fn save_session_filters(session_id: i32, sync_filters: String) -> Result<(), String> {
    let mut cfg = load_session_config(session_id).unwrap_or_default();
    cfg.sync_filters = if sync_filters.is_empty() { None } else { Some(sync_filters) };
    save_session_config_inner(session_id, &cfg).map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn delete_session_config(session_id: i32) -> Result<(), String> {
...
#[tauri::command]
pub async fn get_session_filters(session_id: i32) -> Result<String, String> {
    let cfg = load_session_config(session_id);
    Ok(cfg.and_then(|c| c.sync_filters).unwrap_or_default())
}

#[tauri::command]
pub async fn save_session_info(session_id: i32, info: serde_json::Value) -> Result<(), String> {
    let mut cfg = load_session_config(session_id).unwrap_or_default();
...
#[tauri::command]
pub async fn start_auto_sync(sys_session_id: i32, wx_dir: String, base_url: String, token: Option<String>) -> Result<(), String> {
    let root = PathBuf::from(&wx_dir);
    if !root.exists() {
        return Err("本机未找到会话目录，无法开启自动同步".into());
    }
    // stop existing watcher if any
    let _ = stop_auto_sync(sys_session_id).await;

    let cancel = Arc::new(AtomicBool::new(false));
    let handle = WatchHandle { cancel: cancel.clone() };
    WATCHERS.lock().insert(sys_session_id.to_string(), handle);

    spawn_watcher(sys_session_id, root, base_url, token, cancel).map_err(|e| e.to_string())?;

    // persist auto_sync=true
    let mut cfg = load_session_config(sys_session_id).unwrap_or_default();
    cfg.auto_sync = Some(true);
    save_session_config_inner(sys_session_id, &cfg).map_err(|e| e.to_string())?;

    Ok(())
}

#[tauri::command]
pub async fn stop_auto_sync(sys_session_id: i32) -> Result<(), String> {
    if let Some(w) = WATCHERS.lock().remove(&sys_session_id.to_string()) {
        w.cancel.store(true, Ordering::Relaxed);
    }
    // persist auto_sync=false
    let mut cfg = load_session_config(sys_session_id).unwrap_or_default();
...
#[tauri::command]
pub async fn get_auto_sync_state(session_id: i32) -> Result<bool, String> {
    let cfg = load_session_config(session_id);
